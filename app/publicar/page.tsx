...
export default function PublicarPage() {
  ...
  useEffect(() => {
    if (!loading && !session) router.replace('/auth?next=/publicar');
  ...
  async function publish() {
    setError('');
    if (!city.trim() || !zone.trim() || !court.trim() || !date || !time || !price) {
  ...
  if (loading || !session) return <SplashLoading />;

  return (
    <div className="pb-10">
      {showSuccess && <SuccessCheck message="¡Partido publicado!" donate />}
      <div className="relative overflow-hidden px-5 pb-6 pt-7 text-white">
      ...
      <div className="space-y-4">
        ...
        <Field label="Zona">
          <input className="input" value={zone} onChange={(e) => setZone(e.target.value)} placeholder="Ej: Barrio Sur" />
        </Field>
        <Field label="Cancha">
        ...
        {matchType === 'jugadores_sueltos' && (
          <Field label="Jugadores faltantes">
            <div className="flex items-center gap-4">
              <button type="button" onClick={() => setMissing((m) => Math.max(1, m - 1))} className="stepper-btn">−</button>
              <span className="font-display text-xl font-extrabold">{missing}</span>
              <button type="button" onClick={() => setMissing((m) => Math.min(10, m + 1))} className="stepper-btn">+</button>
            </div>
          </Field>
        )}
        ...
        <button
          onClick={publish}
          disabled={submitting}
          className="press-fx mt-2 w-full rounded-2xl bg-brand py-4 font-display font-bold text-white shadow-lg shadow-brand/30 disabled:opacity-60"
        >
