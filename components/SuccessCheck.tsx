...
export default function SuccessCheck({ message, donate = false }: { message: string; donate?: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-ink/30">
      <div className="pop-in flex flex-col items-center gap-3 rounded-3xl bg-white px-8 py-7 shadow-xl">
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-brand-pale">
        ...
        <p className="text-center font-display text-sm font-bold text-ink">{message}</p>
        {donate && (
          <a
            href={DONATE_URL}
            target="_blank"
            ...
            className="press-fx mt-1 rounded-full bg-[#00B1EA] px-4 py-2 text-xs font-bold text-white"
          >
            {phrase}
          </a>
        )}
