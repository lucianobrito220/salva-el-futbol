    ...
    <>
      ...
      {isTopLevel && (
        <button
          ...
          className="fixed left-4 top-[10px] z-40 flex h-11 w-11 items-center justify-center rounded-full bg-white text-ink shadow-md backdrop-blur-md transition-transform hover:scale-105 active:scale-95"
        >
          <Menu size={24} />
        </button>
      )}
      ...
      <div
        className={`fixed bottom-0 left-0 top-0 z-50 w-72 max-w-[80vw] transform bg-white/90 dark:bg-neutral-900/95 backdrop-blur-xl shadow-2xl transition-transform duration-300 ease-in-out ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-center justify-between border-b border-line dark:border-neutral-800 px-5 py-4">
    ...
    <Link
      ...
      className={`flex items-center gap-3 rounded-xl px-4 py-3.5 transition-colors ${
        highlight
          ...
          : 'font-medium text-ink dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-800'
      }`}
    >
